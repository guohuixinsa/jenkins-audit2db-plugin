...
public class BuildDetailsHibernateRepository extends
	...
	@Override
	public BuildJob getBuildJobByName(final String name) {
		if (null == name ) {
			throw new IllegalArgumentException("Invalid name: cannot be null.");
		}

		BuildJob retval = null;

		final DetachedCriteria criteria = DetachedCriteria
				.forClass(BuildJob.class);
		criteria.add(Restrictions.eq("jobName", name).ignoreCase());

		try {
			@SuppressWarnings("unchecked")
			final List<BuildJob> jobs = getHibernateTemplate()
					.findByCriteria(criteria);
	...
	@Override
	public int getCountBuildDetailsByName(String name) {
		if (null == name ) {
			throw new IllegalArgumentException("Invalid name: cannot be null.");
		}
		int retval = 0;
	...
	@Override
	public int getCountBuildSuccessByName(String name) {
		if (null == name ) {
			throw new IllegalArgumentException("Invalid name: cannot be null.");
		}
		int retval = 0;
		...
		criteria.add(Restrictions.eq("name",name).ignoreCase())
				.add(Restrictions.eq("result","SUCCESS"));
		try {
			@SuppressWarnings("unchecked")
			List<BuildDetails> buildList = getHibernateTemplate().findByCriteria(criteria);
			retval =  buildList.size();
    ...
    @Override
    public Object saveBuildJob(BuildJob buildjob){
        if(null == buildjob ){
            throw new IllegalArgumentException(
                    "Invalid build buildjob: cannot be null.");
        }
        String name = buildjob.getJobName();
        BuildJob jobname = getBuildJobByName(name);
        return getHibernateTemplate().save(buildjob);
    }

    @Override
	public Object savnBuildChange(BuildChange buildChange)
	{
		if(null == buildChange){
			throw new IllegalArgumentException(
					"Invalid builf builfchange cannot be null." );
		}
		return getHibernateTemplate().save(buildChange);
	}

	@Override
	public     Object saveChangeFile(ChangeFile changfile){
    	if(null==changfile ){
			throw new IllegalArgumentException(
					"Invalid builf builfchange cannot be null." );
		}
		return getHibernateTemplate().save(changfile);
     ...
     */
    @Override
    public BuildDetails getBuildDetailsById(final int id) {
	return getHibernateTemplate().get(BuildDetailsImpl.class, id);
    }

	//public BuildDetails getBuildDetailsByFullName(final String fullname ) {
	//	return getHibernateTemplate().get(BuildDetailsImpl.class, fullName);
	//}
    ...
    public List<BuildDetails> getBuildDetailsByBuildNum(int   buildNum){
        final DetachedCriteria criteria = DetachedCriteria
                .forClass(BuildDetails.class);
        criteria.add(Restrictions.eq("buildNum", buildNum));
        return getHibernateTemplate().findByCriteria(criteria);
    }

    public List<BuildDetails> getBuildDetailsBySvnUrl(String svnUrl){
        final DetachedCriteria criteria = DetachedCriteria
                .forClass(BuildDetails.class);
        criteria.add(Restrictions.ilike("svnUrl", svnUrl, MatchMode.EXACT));
        return getHibernateTemplate().findByCriteria(criteria);
    }

    public List<BuildDetails> getBuildDetailsBySvnRev(String svnRev){
        final DetachedCriteria criteria = DetachedCriteria
                .forClass(BuildDetails.class);
        criteria.add(Restrictions.ilike("svnRev", svnRev, MatchMode.EXACT));
        return getHibernateTemplate().findByCriteria(criteria);
    }
    public List<BuildDetails> getBuildDetailesByJobId(int jobId){
        final DetachedCriteria criteria = DetachedCriteria
                .forClass(BuildDetails.class);
	...
	final String fullname = new BuildDetailsImpl(build).getFullName();
	return getBuildDetailsByFullName(fullname).get(0);
    }

    @Override
    public BuildJob getBuildJobForBuild(final AbstractBuild<?, ?> build) {
        final BuildJob job  = new BuildJobImpl(build);
        return job;
    }
